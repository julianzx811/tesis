...
for (var i = 0; i < request.length; i++) {
  ...
  request[i]["categoria"] = 8;
  request[i]["Disponibilidad"] = "online";
  if ("Description" in request[i]) {
    request[i]["Descripcion"] = request[i]["Description"];
  } else if ("Plot" in request[i]) {
    request[i]["Descripcion"] = request[i]["Plot"];
  }
  if ("BoxOfficeEarnings" in request[i]) {
    request[i]["precio"] = request[i]["BoxOfficeEarnings"];
  } else if ("Revenue (Millions)" in request[i]) {
    request[i]["precio"] = request[i]["Revenue (Millions)"];
  }
  request[i]["link"] = request[i]["Director"];
  if ("Imagen" in request[i]) {
    request[i]["Imagen"] = request[i]["Actors"];
  } else {
    request[i]["Imagen"] = "imagen.jpg";
  }

  // Eliminar los atributos con los nombres antiguos
  delete request[i]["ID"];
  delete request[i]["Cast"];
  delete request[i]["ReleaseYear"];
  delete request[i]["IMDbRating"];
  delete request[i]["IMDbVotes"];
  delete request[i]["Title"];
  delete request[i]["Genres"];
  delete request[i]["Director"];
  delete request[i]["Duration"];
  delete request[i]["BoxOfficeEarnings"];
  delete request[i]["Metascore"];
  delete request[i]["Rank"];
  delete request[i]["Genre"];
  delete request[i]["Actors"];
  delete request[i]["Runtime (Minutes)"];
  delete request[i]["Revenue (Millions)"];
  delete request[i]["Votes"];
  delete request[i]["Year"];
  delete request[i]["Rating"];
  delete request[i]["Plot"];
  delete request[i]["Description"];
}
var jsonString = JSON.stringify(request, null, 2).replace(
  /"([^"]+)":/g,
  '"$1":'
);

fs.writeFile("Lsa_model.json", jsonString, (err) => {
  if (err) {
    console.error(err);
  }

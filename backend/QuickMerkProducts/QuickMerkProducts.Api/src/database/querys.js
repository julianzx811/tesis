const querys = {
  ...
  getProductInfo:
    "select * from Producto as pd,Producto_info as pdi where pd.ProductId = @Id and pdi.ProductInfoId = @Id",
  ...
  GetProductByName:
    "SELECT * FROM Producto WHERE ProductName LIKE  CONCAT('%', @ProductoName, '%')",
  PostProductInfo:
    "INSERT INTO Producto_info(precio,Disponibilidad,Imagen,Descripcion,categoria,link) VALUES (@precio,@Disponibilidad,@Imagen,@Descripcion,@categoria,@link); SELECT SCOPE_IDENTITY() AS id;",
  PostProduct:
  ...
  GetProductByCategoria:
    "SELECT DISTINCT p.*, pc.* FROM Producto AS p INNER JOIN Producto_info AS pc ON p.info = pc.ProductInfoId where pc.categoria = @categoria;",
};
